...
router.get('/', (req, res) => {
  ...
  if (query) {
    const foundStocks = stocks.filter(
      ...
      (stock) => stock.code.toLowerCase().indexOf(query) != -1);
...
router.patch('/:code', (req, res) => {
  ...
  if (foundStock) {
    ...
    foundStock.name = req.body.name;
    //foundStock.code = req.body.code;
    foundStock.previousPrice = req.body.price == foundStock.price ? foundStock.previousPrice : foundStock.price;
    foundStock.price = req.body.price;
    foundStock.exchange = req.body.exchange;
